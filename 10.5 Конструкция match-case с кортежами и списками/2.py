"""
Вводятся данные по книге в виде строки через запятую в формате
...
book = [t[i](x) if t[i] != str else x.strip()
...
При срабатывании шаблона вывести на экран строку "Yes". Если ни один
из шаблонов не сработал, то вывести строку "No".
...
book = [t[i](x) if t[i] != str else x.strip()
        for i, x in enumerate(input().split(","))]
match book:
    case (int(), str() as author, str() as title
          ) if len(author) >= 6 and len(title) >= 10:
        print("Yes")
    case (int(), str() as author, str() as title, float() as price
          ) if len(author) >= 6 and price > 0:
        print("Yes")
    case (int(), str() as author, str() as title, int() as year
          ) if year >= 2020:
        print("Yes")
    case (int(), str() as author, str() as title, float() as price,
